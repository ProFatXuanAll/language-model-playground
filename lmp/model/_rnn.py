...
class RNNModel(BaseModel):
  ...
  Use ``self.loss_fn`` for training and use ``self.pred`` for inference.  Both are depended on forward pass alogorithm
  ``self.forward``.

  Parameters
  ----------
  d_emb: int
    Token embedding dimension.  Must be bigger than or equal to ``1``.
  d_hid: int
    Hidden dimension for Feed-Forward layers and RNN layers.  Must be bigger than or equal to ``1``.
  kwargs: Dict, optional
    Useless parameter.  Intently left for subclass inheritance.
  n_hid_lyr: int
    Number of RNN layers.  Must be bigger than or equal to ``1``.
  n_post_hid_lyr: int
    Number of Feed-Forward layers after RNN layers.  All layers are paired with ReLU activatons except for the last
    one.  Must be bigger than or equal to ``1``.
  n_pre_hid_lyr: int
    Number of Feed-Forward layers before RNN layers.  All layers are paired with ReLU activatons.  Must be bigger than
    or equal to ``1``.
  p_emb: float
    Dropout probability for token embeddings.  Must satisfy ``0.0 <= p_emb <= 1.0``.
  p_hid: float
    Dropout probability for every hidden representations.  Must satisfy ``0.0 <= p_hid <= 1.0``.
  ...
  emb_dp: torch.nn.Dropout
    Token embedding dropout.  Drop embedding features with probability ``p_emb``.
  hid: torch.nn.RNN
    Vanilla RNN which encode temporal features.  Each time step's hidden state depends on current input and previous
    hidden state.  Dropout temporal features with probability ``p_hid`` if ``n_hid_lyr > 1``.
  model_name: ClassVar[str]
    Model name is ``RNN``.  Used for command line argument parsing.
  post_hid: torch.nn.Sequential
    Rectified Feed-Forward layers which transform temporal features from  hidden dimension ``d_hid`` to embedding
    dimension ``d_emb``.  Drop rectified units with probability ``p_hid``.
  pre_hid: torch.nn.Sequential
    Rectified Feed-Forward layers which transform token embeddings from embedding dimension ``d_emb`` to hidden
    dimension ``d_hid``.  Drop rectified units with probability ``p_hid``.
  """
  ...
  def __init__(
    ...
    **kwargs: Optional[Dict],
    ...
    if n_hid_lyr < 1:
      raise ValueError('`n_hid_lyr` must be bigger than or equal to `1`.')
    ...
    if n_post_hid_lyr < 1:
      raise ValueError('`n_post_hid_lyr` must be bigger than or equal to `1`.')
    ...
    if n_pre_hid_lyr < 1:
      raise ValueError('`n_pre_hid_lyr` must be bigger than or equal to `1`.')
    ...
    if not (0.0 <= p_emb <= 1.0):
      raise ValueError('`p_emb` must be in the range from `0.0` to `1.0`.')
    ...
    if not (0.0 <= p_hid <= 1.0):
      raise ValueError('`p_hid` must be in the range from `0.0` to `1.0`.')
    ...
    self.loss_ignore_padding = nn.CrossEntropyLoss(ignore_index=tknzr.pad_tkid)
  ...
  def forward(self, batch_prev_tkids: torch.Tensor) -> torch.Tensor:
    ...
    #. Input batch of previous token ids.
       (shape: ``(B, S)``)
    #. Use batch of previous token ids to perform token embeddings lookup on ``self.emb``.
       (shape: ``(B, S, E)``)
    #. Use ``self.emb_dp`` to drop some features in token embeddings.
       (shape: ``(B, S, E)``)
    #. Use ``self.pre_hid`` to transform token embeddings from embedding dimension ``E`` to hidden dimension ``H``.
       (shape: ``(B, S, H)``)
    #. Use ``self.hid`` to encode temporal features.
       (shape: ``(B, S, H)``)
    #. Use ``self.post_hid`` to transform temporal features from hidden dimension ``H`` to embedding dimension ``E``.
       (shape: ``(B, S, E)``)
    #. Find the most possible next token id in embedding matrix ``self.emb`` using inner product.  This reduce
       parameters since we share weight on token embedding and output projection.
       (shape: ``(B, S, V)``)
    #. Return logits.  Used with ``self.pred`` to convert logit into prediction.  Used wtih ``self.loss_fn`` to perform
       optimization.
       (shape: ``(B, S, V)``)
    ...
    -------
    torch.Tensor
      Next token logits for each token id in batch.  Logits has shape ``(B, S, V)`` and ``dtype == torch.float32``.
    """
  ...
  def loss_fn(self, batch_next_tkids: torch.Tensor, batch_prev_tkids: torch.Tensor) -> torch.Tensor:
    ...
    Use forward pass to get logits and then use cross-entropy to calculate next token prediction loss.  Use teacher
    forcing to implement this method.
  ...
  def pred(self, batch_prev_tkids: torch.Tensor) -> torch.Tensor:
    ...
    Use forward pass ouput logits to choose the most possible token id from vocabulary as next token.
    ...
    -------
    torch.Tensor
      Softmax predicition for next token.  Return tensor has shape ``(B, S, V)`` and ``dtype == torch.float32``.
    """
    # Forward pass.
    # Input  shape: `(B, S)`.
    # Output shape: `(B, S, V)`.
  ...
  def train_parser(parser: argparse.ArgumentParser) -> None:
    """Training vanilla RNN language model CLI arguments parser.
