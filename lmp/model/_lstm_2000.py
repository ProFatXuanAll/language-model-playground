...
class LSTM2000(BaseModel):
  ...
  +------------------+----------------------------+ :math:`f\tn`,        |                     |
  | :math:`W^i`,     | ``(n_blk, d_emb)``         | :math:`o\tn`         |                     |
  | :math:`W^f`,     |                            |                      |                     |
  | :math:`W^o`      |                            |                      |                     |
  +------------------+----------------------------+----------------------+---------------------+
  | :math:`U^i`,     | ``(n_blk, n_blk x d_blk)`` | :math:`i_k\tn`,      | ``(1)``             |
  | :math:`U^f`,     |                            | :math:`f_k\tn`,      |                     |
  | :math:`U^o`      |                            | :math:`o_k\tn`       |                     |
  +------------------+----------------------------+----------------------+---------------------+
  | :math:`b^i`,     | ``(n_blk)``                | :math:`g^\ck\tn`,    | ``(d_blk)``         |
  ...
  def forward(self, batch_cur_tkids: torch.Tensor, batch_next_tkids: torch.Tensor) -> torch.Tensor:
    ...
    for i in range(seq_len):
      ...
      # Get forget gates.
      # shape: (batch_size, n_blk, 1)
      forget_gates = cells_and_gates_input_act[:, d_hid + self.n_blk:d_hid + 2 * self.n_blk].unsqueeze(2)
      ...
      # Get output gates.
      # shape: (batch_size, n_blk, 1)
      output_gates = cells_and_gates_input_act[:, d_hid + 2 * self.n_blk:].unsqueeze(2)
