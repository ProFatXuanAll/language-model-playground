...
class ElmanNet(BaseModel):
  ...
  - Our implementation use :math:`\tanh` as activation function instead of sigmoid function (which is used in the
    paper).  The concern here is that optimal embeddings may have negative values.
  ...
  def pred(
    ...
    # Token embedding lookup and project to recurrent units. Then perform recurrent calculation.
    # shape: (batch_size, d_hid).
    h_cur = torch.tanh(self.proj_e2h(self.emb(batch_cur_tkids)) + self.proj_h2h(h_prev))

    # Project hidden states to embedding dimension. Then calculate similarity scores by calculating inner product over
    ...
    # shape: (batch_size, vocab_size).
    return (F.softmax(sim, dim=1), [h_cur])
