...
class BaseTokenizer:
    def __init__(self, pad_token: str = '[PAD]', pad_token_id: int = 0,
    ...
    def load_from_file(cls, file_path: str = None):
        ...
        self.id_to_token = {i: v for i, v in enumerate(self.token_to_id)}
        return self
    ...
    def build_dict(self, all_sentences: List[str], min_count: int = 0, uncase: bool = False):
        if uncase:  # 如果uncase是True，就不分大小寫
...
class CharTokenizerDict(BaseTokenizerDict):
    def __init__(self, **kwargs):
