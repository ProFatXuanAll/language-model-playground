"""Evaluate language model on dataset.

Tool for evaluating language model on dataset.  Pre-trained model are used to calculate perplexity on dataset.  This
script serve as model validation, and it is usually run after training model.  All evaluation results will be shown on
both CLI and tensorboard.  Use ``pipenv run tensorboard`` to launch tensorboard and use browser to open URL
...
Examples
--------
The following example using ``valid`` version of :py:class:`lmp.dset.WikiText2` dataset to evaluate pre-trained
language model experiment ``my_exp``.  It evaluate on checkpoints start from number ``5000`` to last.
...
Since evaluation do not need to calculate backward pass, model will consume less memory than training, thus we can use
larger batch size by increasing ``--batch_size`` to accelerate evaluation process.
...
def parse_args() -> argparse.Namespace:
  ...
  --ver
    Version of the dataset.  Default to ``dset``'s default version.
  ...
  for dset_name, dset_clss in DSET_OPTS.items():
    ...
    dset_parser.add_argument(
      ...
      help=' '.join(
        [
          'Pre-trained language model first checkpoint to evaluate.  Evaluate all pre-trained language model',
          'checkpoints range from `--first_ckpt` to `--last_ckpt`.',
        ]
      ),
      required=True,
      type=int,
    ...
    dset_parser.add_argument(
      ...
      help=' '.join(
        [
          'Pre-trained language model last checkpoint to evaluate.  Evaluate all pre-trained language model',
          'checkpoints range from `--first_ckpt` to `--last_ckpt`.  Last checkpoint is also included to evaluate.',
        ]
      ),
      type=int,
    )
    dset_parser.add_argument(
      '--ver',
      default=None,
      help=f'Version of the {dset_name} dataset.  Defaults to {dset_clss.df_ver}.',
...
def main() -> None:
  ...
  dldr = torch.utils.data.DataLoader(dataset=dset, batch_size=args.batch_size, shuffle=False)
  ...
  for ckpt in lmp.util.model.list_ckpts(exp_name=args.exp_name, first_ckpt=args.first_ckpt, last_ckpt=args.last_ckpt):
    ...
    model = lmp.util.model.load(ckpt=ckpt, tknzr=tknzr, **model_cfg.__dict__)
