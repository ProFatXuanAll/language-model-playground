...
Tool for training tokenizer on particular dataset.  This script is usually run before training model.
...
Examples
--------
The following example train :py:class:`lmp.tknzr.WsTknzr` (``whitespace``) on :py:class:`lmp.dset.WikiText2Dset` using
...
The training result will be save at ``exp/my_exp``, and can be reused by other scripts.
...
Use ``--is_uncased`` to avoid differ tokens with same charaters but in different case.
...
def parse_arg() -> argparse.Namespace:
  ...
  Argument must begin with a tokenizer name ``tknzr_name``.  All arguments are added with tokenizer's static method
  ``train_parser``.
  ...
  """
  # Create parser.
  parser = argparse.ArgumentParser('python -m lmp.script.train_tokenizer', description='Train tokenizer.')
