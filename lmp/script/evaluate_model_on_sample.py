"""Evaluate language model on given sample.

Tool for evaluating language model on given sample.  Pre-trained model are used to calculate perplexity on given
sample.  This script serve as model validation, and it is usually run after training model.
...
Examples
--------
The following example use ``"Hello world"`` as input sample to evaluate pre-trained language model experiment
``my_exp``.  It evaluate on the checkpoint number ``5000``.
