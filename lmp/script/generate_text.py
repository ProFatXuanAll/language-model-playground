...
Automatically generate text with pred-trained language model condition on given text segment.  Pre-trained model are
used to generate text.  This script serve as model validation, and it is usually run after training model.
...
Examples
--------
The following example use ``"Hello world"`` as conditional text segment to generate text with pre-trained language
model experiment ``my_exp``.  It use ``top-1`` as inference method to generate text.  It load pre-trained language
model on checkpoint number ``5000``.
...
def parse_args() -> argparse.Namespace:
  ...
  for infer_name, infer_clss in INFER_OPTS.items():
    ...
    infer_parser = subparsers.add_parser(infer_name, description=f'Use {infer_name} as inference method.')
...
def main() -> None:
  ...
  model = lmp.util.model.load(ckpt=args.ckpt, tknzr=tknzr, **model_cfg.__dict__)

  # Get inference method.
  infer = lmp.util.infer.create(max_seq_len=model_cfg.max_seq_len, **args.__dict__)
