...
Tool for training language model on particular dataset.  This script is usually run after training tokenizer.  Training
performance will be shown on both CLI and tensorboard.  Use ``pipenv run tensorboard`` to launch tensorboard and use
browser to open URL http://localhost:6006/ to see training performance.
...
Examples
--------
The following example train :py:class:`lmp.model.RNNModel` (``RNN``) on :py:class:`lmp.dset.WikiText2Dset` using
``train`` version (``--dset_name wiki-text-2`` and ``--ver train``).
...
The training result will be save at ``exp/my_model_exp``, and can be reused by other scripts.  We only save checkpoint
for each ``--ckpt_step`` step and log performance for each ``--log_step``.

One can train more epochs by increasing ``--n_epoch``, but be careful model might be overfitting if trained to much
epochs.
...
- Reduce model parameters (In :py:class:`lmp.model.RNNModel` this means ``--d_emb``, ``--d_hid``, ``n_hid_lyr``,
  ``n_post_hid_lyr`` and ``n_pre_hid_lyr``).
- Use dropout (In :py:class:`lmp.model.RNNModel` this means ``--p_emb`` and ``--p_hid``).
...
``--wd`` to adjust optimizer hyper-parameters.  We also use ``--max_norm`` to avoid gradient explosion.
...
def parse_arg() -> argparse.Namespace:
  ...
  for model_name, model_clss in lmp.model.MODEL_OPTS.items():
    ...
    model_parser = subparsers.add_parser(model_name, description=f'Training {model_name} language model.')
...
def main() -> None:
  ...
  dldr = torch.utils.data.DataLoader(dataset=dset, batch_size=args.batch_size, shuffle=True)
  ...
  optim = torch.optim.AdamW(optim_group_params, betas=(args.beta1, args.beta2), lr=args.lr, eps=args.eps)
  ...
  for epoch in range(args.n_epoch):
    tqdm_dldr = tqdm(dldr, desc=f'epoch: {epoch}, loss: {pre_avg_loss:.6f}')
    for batch_txt in tqdm_dldr:
      # Encode batch text into batch token ids.
      batch_tkids = tknzr.batch_enc(batch_txt=batch_txt, max_seq_len=args.max_seq_len)
      ...
      loss = model.loss_fn(batch_next_tkids=batch_next_tkids, batch_prev_tkids=batch_prev_tkids)
      ...
      if step % args.log_step == 0:
        ...
        # Log on CLI.
        tqdm_dldr.set_description(f'epoch: {epoch}, loss: {avg_loss:.6f}')

        # Log on tensorboard
        writer.add_scalar(f'loss/{args.dset_name}/{args.ver}', avg_loss, step)
