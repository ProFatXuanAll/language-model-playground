...
class Top1Infer(BaseInfer):
  ...
  Use index with maximum probability as next token id prediction.  It is a greedy algorithm, simple but lack of
  dynamic.

  Attributes
  ----------
  infer_name: ClassVar[str]
    Inference method name is ``top-1``.  Used for command line argument parsing.
  """
  ...
  def gen(self, model: BaseModel, tknzr: BaseTknzr, txt: str) -> str:
    ...
    #. Use for-loop to generate sequence of token ids.
       ...
       #. Get the last next token id probability distribution.
          (shape: ``(1, V)``)
       #. Get the index with maximum probability as the last next token id prediction result.
          (shape: ``(1, 1)``)
