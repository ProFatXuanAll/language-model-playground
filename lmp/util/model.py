...
def create(model_name: str, **kwargs: Optional[Dict]) -> BaseModel:
  ...
  Create new language model based on ``model_name``.  All keyword arguments are collected in ``**kwargs`` and are
  passed directly to model's ``__init__`` method.
  ...
  kwargs: Dict, optional
    Model's specific hyperparameters.  All model specific hyperparameters must be passed in as keyword arguments.
...
def load(ckpt: int, exp_name: str, model_name: str, **kwargs: Optional[Dict]) -> BaseModel:
  """Load pre-trained language model.

  Load pre-trained language model from experiment ``exp_name``.  Language model instance is load based on
  ``model_name`` and ``ckpt``.
...
def list_ckpts(exp_name: str, first_ckpt: int, last_ckpt: int) -> List[int]:
  ...
  List all pre-trained model checkpoint from ``first_ckpt`` to ``last_ckpt``.  Last checkpoint is also included. All
  checkpoints must named with format ``r'model-\d+.pt'``.  If ``first_ckpt == -1``, then only include last checkpoint.
  If ``last_ckpt == -1``, then include all checkpoint start after ``first_ckpt``.
  ...
  -------
  List[int]
    All available checkpoints from ``first_ckpt`` to ``last_ckpt``.  Checkpoint are sorted in ascending order.
  ...
  """
  ckpts = []
  for ckpt in os.listdir(os.path.join(lmp.util.path.EXP_PATH, exp_name)):
