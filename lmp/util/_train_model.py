...
def train_model(
    ...
    for cur_epoch in range(epoch):
        ...
        for x, y in epoch_iterator:
            ...
            # shape (B x S) for cross-entropy. Since `view` require tensor to
            # be contigous in memory, `y` need to first reallocate memory using
            # `contiguous()`.
            # x.size = (B, S)
            # y.size = (B x S)
            x = x.to(device)
...
def train_model_by_config(
    ...
    collate_fn = lmp.dataset.BaseDataset.creat_collate_fn(
        tokenizer=tokenizer,
