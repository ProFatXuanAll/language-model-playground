...
def test_parameters(d_blk: int, d_emb: int, n_blk: int, tknzr: BaseTknzr, lstm_2002: LSTM2002) -> None:
  ...
  assert lstm_2002.proj_e2c.weight.size() == torch.Size([n_blk * (3 + d_blk), d_emb])
  assert lstm_2002.proj_e2c.bias.size() == torch.Size([n_blk * (3 + d_blk)])
  ...
  assert torch.all(
    (-inv_sqrt_dim <= lstm_2002.proj_e2c.bias[d_hid:d_hid + n_blk]) &
    (lstm_2002.proj_e2c.bias[d_hid:d_hid + n_blk] <= 0)
  )
  assert torch.all(
    (0 <= lstm_2002.proj_e2c.bias[d_hid + n_blk:d_hid + 2 * n_blk]) &
    (lstm_2002.proj_e2c.bias[d_hid + n_blk:d_hid + 2 * n_blk] <= inv_sqrt_dim)
  )
  assert torch.all(
    (-inv_sqrt_dim <= lstm_2002.proj_e2c.bias[d_hid + 2 * n_blk:]) & (lstm_2002.proj_e2c.bias[d_hid + 2 * n_blk:] <= 0)
  )

  assert hasattr(lstm_2002, 'proj_h2c')
  assert isinstance(lstm_2002.proj_h2c, nn.Linear)
  assert lstm_2002.proj_h2c.weight.size() == torch.Size([n_blk * (3 + d_blk), n_blk * d_blk])
  ...
  assert lstm_2002.proj_h2e.weight.size() == torch.Size([d_emb, n_blk * d_blk])
