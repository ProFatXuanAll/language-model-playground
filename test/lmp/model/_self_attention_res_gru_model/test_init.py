r"""Test `lmp.model.SelfAttentionResGRUModel.__init__`.

Usage:
    python -m unittest test.lmp.model._self_attention_res_gru_model.test_init
"""
...
from lmp.model import SelfAttentionResGRUModel


class TestInit(unittest.TestCase):
    r"""Test case for `lmp.model.SelfAttentionResGRUModel.__init__`."""
    ...
    def setUp(self):
        r"""Setup hyperparameters and construct `SelfAttentionResGRUModel`."""
        self.model_objs = []
        cls = self.__class__
        ...
        for (
            ...
            if vocab_size <= pad_token_id:
                continue
            model = SelfAttentionResGRUModel(
                d_emb=d_emb,
                d_hid=d_hid,
                dropout=dropout,
    ...
    def test_signature(self):
        ...
        self.assertEqual(
            inspect.signature(SelfAttentionResGRUModel.__init__),
            inspect.Signature(
                parameters=[
                    inspect.Parameter(
                        name='self',
    ...
    def test_instance_attributes(self):
        ...
        examples = (
            ...
            ('rnn_layer', torch.nn.Sequential),
            ('proj_hid_to_emb', torch.nn.Sequential),
            ('proj_query', torch.nn.Linear),
            ('proj_key', torch.nn.Linear),
            ('proj_value', torch.nn.Linear),
        )
    ...
    def test_residual_rnn_layer(self):
        r"""Declare correct residual GRU layer(s)."""
        msg = 'Must declare correct residual GRU layer(s).'
        examples = (
            (
        ...
        for rnn_layer, d_hid, dropout, num_rnn_layers in examples:
            self.assertEqual(len(rnn_layer), num_rnn_layers, msg=msg)
            for layer in rnn_layer:
                self.assertIsInstance(layer, ResGRUBlock, msg=msg)
                self.assertEqual(layer.rnn_layer.input_size, d_hid, msg=msg)
                self.assertEqual(layer.rnn_layer.hidden_size, d_hid, msg=msg)
                self.assertEqual(layer.dropout.p, dropout, msg=msg)
    ...
    def test_proj_query(self):
        r"""Declare correct query projection layer."""
        msg = 'Must declare correct query projection layer.'
        examples = (
            (
                model_obj['model'].proj_query,
                model_obj['d_hid'],
            )
            for model_obj in self.model_objs
        )

        for proj_layer, d_hid in examples:
            self.assertIsInstance(proj_layer, torch.nn.Linear, msg=msg)
            self.assertEqual(proj_layer.in_features, d_hid, msg=msg)
            self.assertEqual(proj_layer.out_features, d_hid, msg=msg)

    def test_proj_key(self):
        r"""Declare correct key projection layer."""
        msg = 'Must declare correct key projection layer.'
        examples = (
            (
                model_obj['model'].proj_key,
                model_obj['d_hid'],
            )
            for model_obj in self.model_objs
        )
    ...
    def test_proj_value(self):
        r"""Declare correct value projection layer."""
        msg = 'Must declare correct value projection layer.'
        examples = (
            (
                model_obj['model'].proj_value,
                model_obj['d_hid'],
            )
            for model_obj in self.model_objs
        )
