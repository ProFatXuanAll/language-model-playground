...
def batch_prev_tkids() -> torch.Tensor:
  """Return simple tensor with shape ``(B, S) == (2, 3)``."""
  return torch.zeros((2, 3))


@pytest.fixture
def batch_next_tkids_prob() -> torch.Tensor:
  """Return simple tensor with shape ``(B, S, V) == (2, 3, 4)``."""
  return torch.tensor(
    [
      [
        [0.1, 0.2, 0.3, 0.4],
...
def batch_ppl() -> torch.Tensor:
  """Batch perplexity expected answer."""
  return torch.tensor(
    [
      -1 / 3 * (torch.log(torch.tensor([0.1])) + torch.log(torch.tensor([0.3])) + torch.log(torch.tensor([0.1]))),
...
def subclss_model(batch_next_tkids_prob: torch.Tensor):
  ...
  class SubclssModel(BaseModel):
    ...
    def pred(self, batch_prev_tkids: torch.Tensor) -> torch.Tensor:
      """Return simple tensor with shape ``(B, S, V)``."""
      # (B, S, V) == (2, 3, 4)
