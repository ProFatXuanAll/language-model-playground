...
def test_parameters(d_emb: int, d_hid: int, p_emb: float, p_hid: float, tknzr: BaseTknzr, elman_net: ElmanNet) -> None:
  ...
  assert hasattr(elman_net, 'fc_h2h')
  assert isinstance(elman_net.fc_h2h, nn.Linear)
  assert elman_net.fc_h2h.weight.size() == torch.Size([d_hid, d_hid])
  assert elman_net.fc_h2h.bias is None
  assert torch.all((-inv_sqrt_dim <= elman_net.fc_h2h.weight) & (elman_net.fc_h2h.weight <= inv_sqrt_dim))
  ...
  assert len(elman_net.fc_h2e) == 4
  assert isinstance(elman_net.fc_h2e[0], nn.Dropout)
  assert math.isclose(elman_net.fc_h2e[0].p, p_hid)
  assert isinstance(elman_net.fc_h2e[1], nn.Linear)
  assert elman_net.fc_h2e[1].weight.size() == torch.Size([d_emb, d_hid])
  assert elman_net.fc_h2e[1].bias.size() == torch.Size([d_emb])
  assert torch.all((-inv_sqrt_dim <= elman_net.fc_h2e[1].weight) & (elman_net.fc_h2e[1].weight <= inv_sqrt_dim))
  assert torch.all((-inv_sqrt_dim <= elman_net.fc_h2e[1].bias) & (elman_net.fc_h2e[1].bias <= inv_sqrt_dim))
  assert isinstance(elman_net.fc_h2e[2], nn.Tanh)
  assert isinstance(elman_net.fc_h2e[3], nn.Dropout)
  assert math.isclose(elman_net.fc_h2e[3].p, p_hid)
