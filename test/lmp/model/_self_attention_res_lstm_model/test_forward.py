r"""Test `lmp.model.SelfAttentionResLSTMModel.forward`.

Usage:
    python -m unittest test.lmp.model._self_attention_res_lstm_model.test_forward
"""
...
class TestForward(unittest.TestCase):
    r"""Test case for `lmp.model.SelfAttentionResLSTMModel.forward`."""
    ...
    def setUp(self):
        r"""Setup hyperparameters and construct `_self_attention_res_lstm_model`."""
        self.model_objs = []
        cls = self.__class__
    ...
    def test_signature(self):
        ...
        self.assertEqual(
            inspect.signature(SelfAttentionResLSTMModel.forward),
            inspect.Signature(
                parameters=[
                    inspect.Parameter(
                        name='self',
        ...
        for invalid_input in examples:
            for model_obj in self.model_objs:
                ...
                with self.assertRaises(TypeError, msg=msg):
                    model.forward(batch_sequences=invalid_input)
    ...
    def test_return_size(self):
        ...
        for x, model, vocab_size in examples:
            ...
            self.assertEqual(logits.size(-1), vocab_size)
