r"""Test `lmp.model.ResLSTMBlock.__init__`.

Usage:
    python -m unittest test.lmp.model._res_lstm_block.test_init
"""
...
class TestInit(unittest.TestCase):
    ...
    def setUp(self):
        r"""Setup hyperparameters and construct `ResLSTMBlock`."""
        self.model_objs = []
        cls = self.__class__
    ...
    def test_signature(self):
        ...
        self.assertEqual(
            inspect.signature(ResLSTMBlock.__init__),
            inspect.Signature(
                parameters=[
                    inspect.Parameter(
                        name='self',
    ...
    def test_invalid_input_d_hid(self):
        ...
        for invalid_input in examples:
            with self.assertRaises(
                ...
                ResLSTMBlock(d_hid=invalid_input, dropout=0.0)
    ...
    def test_invalid_input_dropout(self):
        ...
        for invalid_input in examples:
            with self.assertRaises(
                ...
                ResLSTMBlock(d_hid=1, dropout=invalid_input)
    ...
    def test_residual_rnn_layer(self):
        r"""Declare correct residual LSTM block with dropout."""
        msg = 'Must declare correct residual LSTM block with dropout.'
        examples = (
            (
                model_obj['model'],
                model_obj['d_hid'],
        ...
        for model, d_hid, dropout in examples:
            self.assertIsInstance(model.rnn_layer, torch.nn.LSTM, msg=msg)
