...
def test_lstm_2000_layer_parameters(
  ...
  assert torch.all((-inv_sqrt_dim <= lstm_2000_layer.fc_x2fg.weight) & (lstm_2000_layer.fc_x2fg.weight <= inv_sqrt_dim))
  assert torch.all((0 <= lstm_2000_layer.fc_x2fg.bias) & (lstm_2000_layer.fc_x2fg.bias <= inv_sqrt_dim))
  ...
  assert torch.all((-inv_sqrt_dim <= lstm_2000_layer.fc_x2ig.weight) & (lstm_2000_layer.fc_x2ig.weight <= inv_sqrt_dim))
  assert torch.all((-inv_sqrt_dim <= lstm_2000_layer.fc_x2ig.bias) & (lstm_2000_layer.fc_x2ig.bias <= 0))
  ...
  assert torch.all((-inv_sqrt_dim <= lstm_2000_layer.fc_x2og.weight) & (lstm_2000_layer.fc_x2og.weight <= inv_sqrt_dim))
  assert torch.all((-inv_sqrt_dim <= lstm_2000_layer.fc_x2og.bias) & (lstm_2000_layer.fc_x2og.bias <= 0))
  ...
  assert torch.all(
    (-inv_sqrt_dim <= lstm_2000_layer.fc_x2mc_in.weight) & (lstm_2000_layer.fc_x2mc_in.weight <= inv_sqrt_dim)
  )
  assert torch.all(
    (-inv_sqrt_dim <= lstm_2000_layer.fc_x2mc_in.bias) & (lstm_2000_layer.fc_x2mc_in.bias <= inv_sqrt_dim)
  )
  ...
  assert torch.all((-inv_sqrt_dim <= lstm_2000_layer.fc_h2fg.weight) & (lstm_2000_layer.fc_h2fg.weight <= inv_sqrt_dim))
  ...
  assert torch.all((-inv_sqrt_dim <= lstm_2000_layer.fc_h2ig.weight) & (lstm_2000_layer.fc_h2ig.weight <= inv_sqrt_dim))
  ...
  assert torch.all((-inv_sqrt_dim <= lstm_2000_layer.fc_h2og.weight) & (lstm_2000_layer.fc_h2og.weight <= inv_sqrt_dim))
  ...
  assert torch.all(
    (-inv_sqrt_dim <= lstm_2000_layer.fc_h2mc_in.weight) & (lstm_2000_layer.fc_h2mc_in.weight <= inv_sqrt_dim)
  )

  assert hasattr(lstm_2000_layer, 'h_0')
  assert isinstance(lstm_2000_layer.h_0, nn.Parameter)
  assert lstm_2000_layer.h_0.size() == torch.Size([1, n_blk * d_blk])
  assert torch.all((-inv_sqrt_dim <= lstm_2000_layer.h_0) & (lstm_2000_layer.h_0 <= inv_sqrt_dim))

  assert hasattr(lstm_2000_layer, 'c_0')
  assert isinstance(lstm_2000_layer.c_0, nn.Parameter)
  assert lstm_2000_layer.c_0.size() == torch.Size([1, n_blk, d_blk])
  assert torch.all((-inv_sqrt_dim <= lstm_2000_layer.c_0) & (lstm_2000_layer.c_0 <= inv_sqrt_dim))
...
def test_lstm_2000_parameters(
  ...
  assert torch.all((-inv_sqrt_dim <= lstm_2000.fc_e2h[1].weight) & (lstm_2000.fc_e2h[1].weight <= inv_sqrt_dim))
  assert torch.all((-inv_sqrt_dim <= lstm_2000.fc_e2h[1].bias) & (lstm_2000.fc_e2h[1].bias <= inv_sqrt_dim))
  ...
  assert torch.all((-inv_sqrt_dim <= lstm_2000.fc_h2e[0].weight) & (lstm_2000.fc_h2e[0].weight <= inv_sqrt_dim))
  assert torch.all((-inv_sqrt_dim <= lstm_2000.fc_h2e[0].bias) & (lstm_2000.fc_h2e[0].bias <= inv_sqrt_dim))
