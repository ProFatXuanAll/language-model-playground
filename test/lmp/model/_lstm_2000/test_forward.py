...
def test_lstm_2000_layer_forward_path(lstm_2000_layer: LSTM2000Layer, x: torch.Tensor) -> None:
  ...
  for idx in range(x.size(1)):
    c, h = lstm_2000_layer(x=x[:, idx, :].unsqueeze(1), c_0=c_0, h_0=h_0)

    assert isinstance(c, torch.Tensor)
    assert c.size() == torch.Size([B, 1, lstm_2000_layer.n_blk, lstm_2000_layer.d_blk])
    assert c.dtype == torch.float
    assert isinstance(h, torch.Tensor)
    assert h.size() == torch.Size([B, 1, lstm_2000_layer.d_hid])
...
def test_lstm_2000_forward_path(lstm_2000: LSTM2000, batch_cur_tkids: torch.Tensor) -> None:
  """Parameters used during forward pass must have gradients."""
  # Make sure model has zero gradients at the begining.
  ...
  for idx in range(batch_cur_tkids.size(1)):
    logits, batch_cur_states = lstm_2000(
      batch_cur_tkids=batch_cur_tkids[..., idx].reshape(-1, 1),
      batch_prev_states=batch_prev_states,
    )

    assert isinstance(logits, torch.Tensor)
    assert logits.size() == torch.Size([B, 1, lstm_2000.emb.num_embeddings])
