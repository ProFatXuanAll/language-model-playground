"""Test :py:class:`lmp.model._res_rnn` signatures."""
...
def test_inherent_method():
  ...
  assert (inspect.signature(BaseModel.forward) == inspect.signature(ResRNNModel.forward))

  assert (inspect.signature(BaseModel.load) == inspect.signature(ResRNNModel.load))

  assert (inspect.signature(BaseModel.loss_fn) == inspect.signature(ResRNNModel.loss_fn))

  assert (inspect.signature(BaseModel.pred) == inspect.signature(ResRNNModel.pred))

  assert (inspect.signature(BaseModel.ppl) == inspect.signature(ResRNNModel.ppl))

  assert (inspect.signature(BaseModel.save) == inspect.signature(ResRNNModel.save))

  assert (inspect.signature(BaseModel.train_parser) == inspect.signature(ResRNNModel.train_parser))
