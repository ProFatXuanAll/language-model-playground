r"""Test `lmp.model._lstm_model.py`.

Usage:
    python -m unittest test.lmp.model._lstm_model.__init__
"""
...
class TestLSTMModel(unittest.TestCase):
    r"""Test case for `lmp.model._lstm_model.py`."""
    ...
    def test_signature(self):
        ...
        try:
            ...
            import lmp
            import lmp.model
            import lmp.model._lstm_model
            # pylint: enable=C0415

            # pylint: disable=W0212
            self.assertTrue(
                inspect.ismodule(lmp.model._lstm_model),
                msg=msg
            )
            # pylint: enable=W0212
        except ImportError:
    ...
    def test_module_attributes(self):
        ...
        try:
            ...
            for attr in examples:
                self.assertTrue(
                    hasattr(lmp.model._lstm_model, attr),
                    msg=msg1.format(attr)
                )
                self.assertTrue(
                    inspect.isclass(getattr(
                        lmp.model._lstm_model,
                        attr
                    )),
