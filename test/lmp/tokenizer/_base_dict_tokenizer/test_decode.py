r"""Test `lmp.tokenizer.BaseDictTokenizer.decode`.

Usage:
    python -m unittest test/lmp/tokenizer/_base_dict_tokenizer/test_decode.py
"""
...
class TestDecode(unittest.TestCase):
    r"""Test case for `lmp.tokenizer.BaseDictTokenizer.decode`."""
    ...
    def test_signature(self):
        ...
        self.assertEqual(
            inspect.signature(BaseDictTokenizer.decode),
            inspect.Signature(
                parameters=[
                    inspect.Parameter(
                        name='self',
    ...
    def test_abstract_method(self):
        ...
        for is_uncased in examples:
            with self.assertRaises(NotImplementedError, msg=msg1) as ctx_man:
                BaseDictTokenizer(is_uncased=is_uncased).decode([0])
