r"""Test `lmp.tokenizer.BaseListTokenizer.batch_decode`.

Usage:
    python -m unittest \
        test/lmp/tokenizer/_base_list_tokenizer/test_batch_decode.py
"""
...
class TestBatchDecode(unittest.TestCase):
    r"""Test case for `lmp.tokenizer.BaseListTokenizer.batch_decode`."""
    ...
    def test_signature(self):
        ...
        self.assertEqual(
            inspect.signature(BaseListTokenizer.batch_decode),
            inspect.Signature(
                parameters=[
                    inspect.Parameter(
                        name='self',
    ...
    def test_abstract_method(self):
        ...
        for is_uncased in examples:
            with self.assertRaises(NotImplementedError, msg=msg1) as ctx_man:
                BaseListTokenizer(is_uncased=is_uncased).batch_decode([[0]])
