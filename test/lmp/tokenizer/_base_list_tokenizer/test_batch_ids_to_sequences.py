r"""Test `lmp.tokenizer.BaseListTokenizer.batch_ids_to_sequences`.

Usage:
    python -m unittest \
        test/lmp/tokenizer/_base_list_tokenizer/test_batch_ids_to_sequences.py
"""
...
class TestBatchIdsToSequences(unittest.TestCase):
    r"""Test Case for `lmp.tokenizer.BaseListTokenizer.batch_ids_to_sequences`."""
    ...
    def test_signature(self):
        ...
        self.assertEqual(
            inspect.signature(BaseListTokenizer.batch_ids_to_sequences),
            inspect.Signature(
                parameters=[
                    inspect.Parameter(
                        name='self',
    ...
    def test_abstract_method(self):
        ...
        for is_uncased in examples:
            with self.assertRaises(NotImplementedError, msg=msg1) as ctx_man:
                SubClassTokenizer(
                    is_uncased=is_uncased).batch_ids_to_sequences([[0]])
