r"""Test `lmp.tokenizer.BaseDictTokenizer.batch_sequences_to_tokens`.

Usage:
    python -m unittest \
        test/lmp/tokenizer/_char_dict_tokenizer/test_batch_sequences_to_tokens.py
"""
...
class TestBatchSequencesToTokens(unittest.TestCase):
    r"""Test case for `lmp.tokenizer.BaseDictTokenizer.batch_sequences_to_tokens`."""
    ...
    def test_signature(self):
        ...
        self.assertEqual(
            inspect.signature(BaseDictTokenizer.batch_sequences_to_tokens),
            inspect.Signature(
                parameters=[
                    inspect.Parameter(
                        name='self',
    ...
    def test_abstract_method(self):
        ...
        for is_uncased in examples:
            with self.assertRaises(NotImplementedError, msg=msg1) as ctx_man:
                SubClassTokenizer(
                    is_uncased=is_uncased).batch_sequences_to_tokens(['H'])
