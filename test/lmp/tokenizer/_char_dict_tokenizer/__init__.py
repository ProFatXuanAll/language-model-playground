r"""Test `lmp.tokenizer._char_dict_tokenizer.py`.

Usage:
    python -m unittest test/lmp/tokenizer/_char_dict_tokenizer/__init__.py
"""
...
class TestCharDictTokenizer(unittest.TestCase):
    ...
    def test_signature(self):
        ...
        try:
            ...
            import lmp.tokenizer._char_dict_tokenizer
            # pylint: enable=C0415

            # pylint: disable=W0212
            self.assertTrue(
                inspect.ismodule(lmp.tokenizer._char_dict_tokenizer),
                msg=msg
            )
            # pylint: enable=W0212
        except ImportError:
    ...
    def test_module_attributes(self):
        ...
        try:
            ...
            for attr in examples:
                self.assertTrue(
                    hasattr(lmp.tokenizer._char_dict_tokenizer, attr),
                    msg=msg1.format(attr)
                )
                self.assertTrue(
                    inspect.isclass(getattr(
                        lmp.tokenizer._char_dict_tokenizer,
                        attr
                    )),
