r"""Test `lmp.tokenizer.WhitespaceDictTokenizer.batch_ids_to_sequences`.

Usage:
    python -m unittest \
        test/lmp/tokenizer/_whitespace_dict_tokenizer/test_batch_ids_to_sequences.py
"""
...
class TestBatchIdsToSequences(unittest.TestCase):
    r"""Test Case for `lmp.tokenizer.WhitespaceDictTokenizer.batch_ids_to_sequences`."""
    ...
    def test_signature(self):
        ...
        self.assertEqual(
            inspect.signature(WhitespaceDictTokenizer.batch_ids_to_sequences),
            inspect.Signature(
                parameters=[
                    inspect.Parameter(
                        name='self',
        ...
        for invalid_input in examples:
            for tokenizer in self.tokenizers:
                with self.assertRaises(TypeError, msg=msg1) as cxt_man:
                    tokenizer.batch_ids_to_sequences(
    ...
    def test_return_type(self):
        ...
        for batch_token_ids in examples:
            for tokenizer in self.tokenizers:
                batch_sequences = tokenizer.batch_ids_to_sequences(
    ...
    def test_function(self):
        r"""Test token id inverse look up on batch of sequences."""
        ...
        for batch_token_ids, ans_batch_seqs in examples:
            for tokenizer in self.tokenizers:
                self.assertEqual(
                    tokenizer.batch_ids_to_sequences(
                        batch_token_ids=batch_token_ids
