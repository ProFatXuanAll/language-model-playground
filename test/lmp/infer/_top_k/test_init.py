...
def test_max_seq_len():
  """Perform validation on parameter ``max_seq_len``.
    ...
    ``-1`` to ``TopKInfer.hard_max_seq_len``.
    """
  ...
  for bad_max_seq_len in [-2, -3, TopKInfer.hard_max_seq_len + 1]:
    ...
    assert ('`max_seq_len` must be in the range from 0 to ' + f'{TopKInfer.hard_max_seq_len}.' in str(excinfo.value))

  # Test case: correct input.
  for good_max_seq_len in [-1, 0, 1, TopKInfer.hard_max_seq_len]:
    infer = TopKInfer(
      k=1,
    ...
    if good_max_seq_len == -1:
      assert infer.max_seq_len == TopKInfer.hard_max_seq_len
    else:
      assert infer.max_seq_len == good_max_seq_len


def test_k():
  """``k`` must be an instance of ``int`` and larger than ``0``."""
